...
//to-do: use avg of visible spectrum as midpoint for all graphics. 
//instead of mapping vs min and max, map vs +-12 deg from avg and use sidebar to 
//show the feed's current midpoint
...
void RunBoardLEDs() {
	...
	//disable blink if thermal camera running
	//when that happens, this goes solid on
		...
		if (THERMAL_CAMERA_PORTRAIT == 1) {
			...
			for (uint8_t nRow = 0; nRow < 24; nRow++) {	
				...
				for (uint8_t nCol = 4; nCol < 28; nCol++) {				
					...
					//per sensor manufacturer, up to 4 pixels can be dead and it'll still get shipped to YOU!
					if (nCol == 24 && nRow == 14) {
...
//low pass filter attempt, code provided by chatgpt 2023.09.25
void LowPassMicFilter() {	
	//float fRC = 0.001;
	//float fDt = 1.0 / MIC_SAMPLERATE;
	//float fAlpha = fRC / (fRC + fDt);
	//0.9961 -> 26054, 26500 => 26498
	float fAlpha = 0.9411764705882353;
	short nFilteredVal = mnarrSampleData[0];
	
	for (int i = 1; i < mnSamplesRead; i++) {
		nFilteredVal = nFilteredVal + fAlpha * (mnarrSampleData[i] - nFilteredVal);		
		mnarrSampleData[i] = nFilteredVal;
	}
}
