...
void setup() {
	...
	mbAPDSInitialized = oAPDS.begin();	
	
	if (mbAPDSInitialized) {
		//need rgb to cap at 255 for calculations? this isn't limiting shit.
		oAPDS.setIntLimits(0, 255);
		oAPDS.enableColor(true);
		oAPDS.setADCGain(APDS9960_AGAIN_16X);
		oAPDS.enableColorInterrupt();
		oAPDS.setADCIntegrationTime(50);		
	}
	...
	if (nPin == POWER_LED_PIN) {
		...
		if ((lTimer - mnLastUpdateBoard) > mnBoardLEDInterval) {
			...
			switch (mnBoardColor) {
				...
				case 3: ledBoard.setPixelColor(0, 0, 160, 0); mnBoardColor = 2; break;
				case 2: ledBoard.setPixelColor(0, 128, 128, 0); mnBoardColor = 1; break;
				case 1: ledBoard.setPixelColor(0, 160, 64, 0); mnBoardColor = 0; break;
				default: ledBoard.setPixelColor(0, 160, 0, 0); mnBoardColor = 4; break;
			}
...
void RunRGBSensor() {
	if (mbRGBActive) {		
		...
		if (millis() - mnLastRGBScan > mnRGBScanInterval) {
			...
			nTempMax = 0;
			ActivateFlash();
			while(!oAPDS.colorDataReady() || nTempMax == 0) {
				delay(50);
				nTempMax = 1;
			}		
			oAPDS.getColorData(&nRed, &nGreen, &nBlue, &nClear);
